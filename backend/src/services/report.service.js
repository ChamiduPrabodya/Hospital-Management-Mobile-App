...
const generateReportData = async (reportType) => {
  if (reportType === 'appointments') {
    ...
    const counts = await countByStatus({}, 'status', ['pending', 'approved', 'rejected', 'cancelled']);
    return {
      totalAppointments,
  ...
  if (reportType === 'revenue') {
    const completedPayments = await Payment.find({ status: 'completed', isActive: { $ne: false } }).select('amount');
